 ...
 *
 * $Id: SQLizer.hpp,v 1.3 2008-08-27 02:21:42 eric Exp $
 */
...
class SQLizer : public Expressor {
    ...
    public:
	Relation (std::string name): name(name), hasAlias(false) { }
	Relation (std::string name, std::string alias): name(name), hasAlias(true), alias(alias) { }
    };
