// $Id: SWSexScanner.hpp,v 1.3 2008-10-03 07:06:03 eric Exp $

#ifndef SWSexScanner_H
#define SWSexScanner_H
...
#define	YY_DECL						\
    w3c_sw::SWSexParser::token_type				\
    w3c_sw::SWSexScanner::lex(				\
	w3c_sw::SWSexParser::semantic_type* yylval,		\
	w3c_sw::SWSexParser::location_type* yylloc		\
    )
#endif

#ifndef __FLEX_LEXER_H
#define yyFlexLexer SWSexFlexLexer
#include "FlexLexer.h"
#undef yyFlexLexer
#endif
...
/** SWSexScanner is a derived class to add some extra function to the scanner
 * class. Flex itself creates a class named yyFlexLexer, which is renamed using
 * macros to SWSexFlexLexer. However we change the context of the generated
 * yylex() function to be contained within the SWSexScanner class. This is required
 * because the yylex() defined in SWSexFlexLexer has no parameters. */
class SWSexScanner : public SWSexFlexLexer
{
protected:
    SWSexDriver* driver;
...
public:
     ...
     * yylex(). */
    SWSexScanner (SWSexDriver* driver, std::istream* in = NULL, std::ostream* out = NULL)
	: SWSexFlexLexer(in, out), driver(driver)
    {  }

    /** Required for virtual functions */
    virtual ~SWSexScanner () {  }

    /** This is the main lexing function. It is generated by flex according to
     * the macro declaration YY_DECL above. The generated bison parser then
     * calls this virtual function to fetch new tokens. */
    virtual SWSexParser::token_type lex(
	SWSexParser::semantic_type* yylval,
	SWSexParser::location_type* yylloc
	);
    SWSexParser::token_type lexWrapper(
	   SWSexParser::semantic_type* yylval,
	   SWSexParser::location_type* yylloc
					   ) {
	try {
...
protected:
    SWSexParser::token_type typedLiteral (SWSexParser::semantic_type*& yylval, SWSexParser::token_type tok) {
	std::istringstream is(yytext);
	std::ostringstream normalized;

	switch (tok) {
	case w3c_sw::SWSexParser::token::INTEGER:
	    int i;
	    is >> i;
	    normalized << i;
	    yylval->p_NumericRDFLiteral = driver->getNumericRDFLiteral(yytext, i);
	    return tok;
	case w3c_sw::SWSexParser::token::DECIMAL:
	    float f;
	    is >> f;
	    normalized << f;
	    yylval->p_NumericRDFLiteral = driver->getNumericRDFLiteral(yytext, f);
	    return tok;
	case w3c_sw::SWSexParser::token::DOUBLE:
	    double d;
	    is >> d;
	    normalized << d;
    ...
    SWSexParser::token_type unescapeString (SWSexParser::semantic_type*& yylval, SWSexParser::location_type* yylloc, const char* text, size_t len, SWSexParser::token_type tok) {
     ...
     */
    template <typename T> // e.g. SWSexParser::location_type
	...
	try {
	    ...
	    SWSexDriver parser2(stripped, driver->atomFactory);
	    SWSexScanner s(&parser2, &is);
	    SWSexParser::semantic_type yylval2;
	    SWSexParser::location_type yylloc2;
	    SWSexParser::token_type yychar = s.lexWrapper(&yylval2, &yylloc2);
	    if (yychar != SWSexParser::token::IRIREF)
		throw;
#endif
